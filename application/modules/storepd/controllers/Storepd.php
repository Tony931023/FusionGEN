<?php
class Storepd extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('storepd_model');
    }

    public function index()
    {
        /*$t_donacion = $this->storepd_model->ver_donaciones();
        $cout = count($t_donacion);
        var_dump($t_donacion[0]->nombre);*/

        $rdon_db = 0;
        // Definir Termino de uso de las Donaciones

        $t_uso = $this->input->post('t_uso');

        if (isset($t_uso)) {
            $this->storepd_model->update_ron(1, $this->user->getId());
        }

        $ver_tdon = $this->storepd_model->ver_rdon($this->user->getId());
        ...
        $content = $this->template->loadPage("storepd.php", $data);
        $page = $this->template->box('Donaciones', $content);
    ...
    public function saldo()
    {
        ...
        $content = $this->template->loadPage("saldo.php", $data);
        $page = $this->template->box('Donación vía Saldo', $content);
        $this->template->view($page, false, false);
    }
